...
#divRaces <- read_feather("allRes1969toDate.feather") #This Feather file is old and will not be readable beyond the 0.3.0 release #Error in coldataFeather(x, i) : embedded nul in string: 'sday, Aug 25'
...
output$pgl_individual <- renderPlotly({
  ...
  write_csv(pgl(),"problem2.csv") #seemed to work OK
  
  df <- pgl()
  
#   print(glimpse(df))
#   
#   ## issue seems to be that all values are character
# df <-  df %>% 
#     mutate(Gcar=as.integer(Gcar),GSc=as.integer(GSc))
...
```{r}
 ...
 batterWARdata <- eventReactive(input$go5,{
    ...
    # choices <- c("troutmi01","rodrial01") i <- 1
